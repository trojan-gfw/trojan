...
class ipv4_header {
   public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const ipv4_header &header) {
        os.write(reinterpret_cast<const char *>(header.rep_), 20);
        auto options_length = header.header_length() - 20;
        if (options_length < 0) {
            os.setstate(std::ios::failbit);
        }else{
            if (options_length > 0) {
                os.write(reinterpret_cast<const char *>(header.rep_) + 20, options_length);
            }
        } 
        return os;
    }
