...
bool Authenticator::auth(const string &password) {
    ...
    if (mysql_query(&con, ("SELECT quota, download + upload FROM users WHERE fullhash = '" + password + '\'').c_str())) {
...
void Authenticator::record(const std::string &password, uint64_t download, uint64_t upload) {
    ...
    if (mysql_query(&con, ("UPDATE users SET download = download + " + to_string(download) + ", upload = upload + " + to_string(upload) + " WHERE fullhash = '" + password + '\'').c_str())) {
        Log::log_with_date_time(mysql_error(&con), Log::WARN);
    }
}
